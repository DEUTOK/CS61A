...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        a = dice()
        empty_numlist.append(a)
    
    if 1 in empty_numlist:
        return 1
    else: 
        return sum(empty_numlist) 
...
def boar_brawl(player_score, opponent_score):
    ...
    ten_digit = math.floor(opponent_score / 10)
    if ten_digit > 9:
        ten_digit = ten_digit % 10 
    score = 3 * abs(one_digit - ten_digit)
    if score > 1 :
        return score
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != strategy(goal,goal):
                isit = False
    return isit
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    high_num = 1
    max_score = averaged_dice(1,dice)
    for j in range (2,11):
        new_score = averaged_dice(j,dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    #new_score = take_turn(num_rolls, score, opponent_score)
    #new_score = sus_points(score + score)
    #increase = score - new_score

    #new_score = sus_update(num_rolls, score, opponent_score)
    #increase = new_score - score
