...
def pick(paragraphs, select, k):
    ...
    for paragraph in paragraphs:
        if select(paragraph):
            selected_list.append(paragraph)
    if k > len(selected_list)-1:
        return ''
    else: 
...
def wpm(typed, elapsed):
    ...
    avg_words = num_characters / 5
    return avg_words * (60 / elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    least_word = typed_word
    least_value = 9999999
    for i in word_list:
        a = diff_function(typed_word, i, limit)
        if a <= limit:
            if a < least_value:
                least_value, least_word = a, i
    return least_word
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]:  # If the first characters are the same, move to the next characters
        return minimum_mewtations(typed[1:], source[1:], limit)
    
    # If the first characters are different, consider all three operations
    add = minimum_mewtations(typed, source[1:], limit - 1)  # Add the first character of source to typed
    remove = minimum_mewtations(typed[1:], source, limit - 1)  # Remove the first character of typed
    substitute = minimum_mewtations(typed[1:], source[1:], limit - 1)  # Substitute the first character of typed with that of source
    
    # Return the minimum of the three operations, plus one for the operation itself
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for array in timestamps_per_player:
        return_value = []
        for i in range(len(array) - 1):
            return_value += [array[i+1] - array[i]]
        times += [return_value]

    matched_word = match(words, times)
    return matched_word
    # END PROBLEM 9
    ...
    for i in player_indices:
        ...
        for w in word_indices:
            ...
            for j in player_indices:
                if i != j:
                    if times[i][w] > times [j][w]:
                        Best = False
                    elif times[i][w] == times [j][w] and j < i:
                        Best = False
            if Best:
                winning += [words[w]]
