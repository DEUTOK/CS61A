...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        nearest_bee = self.place
        while (not nearest_bee.bees) or (distance_from_bee < self.lower_bound) :

            distance_from_bee = distance_from_bee+ 1

            nearest_bee = nearest_bee.entrance

            

            if nearest_bee.is_hive == True:
                break

        if not nearest_bee.is_hive :
            if distance_from_bee <= self.upper_bound:
                return random_bee(nearest_bee.bees)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            damage_to_bee = self.health + self.damage
            damage_to_self = self.health

        else: 
            damage_to_bee = amount
            damage_to_self = amount

        for bee in list(self.place.bees):
            bee.reduce_health(damage_to_bee)
            
        Ant.reduce_health(self,damage_to_self)
        # END Problem 5
            ...
            if current_place.ant:
                if current_place.ant.is_container:
                    ...
                    if current_place.ant.ant_contained:
                        if not isinstance(current_place.ant.ant_contained, QueenAnt):
                            current_place.ant.ant_contained.double()
                else:
                    if not isinstance(current_place.ant, QueenAnt):
                        current_place.ant.double()
