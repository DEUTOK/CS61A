...
class Server:
    ...
    def send(self, email):
        ...
        recipient_client = self.clients[email.recipient_name]  # This should retrieve a Client object
...
def make_change(amount, coins):
    ...
    # Iterate over each coin denomination, starting with the smallest
    for coin in sorted(coins.keys()):
        if coins[coin] > 0 and coin <= amount:
            # Try using one of this denomination
            new_coins = remove_one(coins, coin)
            result = make_change(amount - coin, new_coins)
            
            # If a valid combination is found, return it with the current coin included
            if result is not None:
                return [coin] + result

    # If no combination can sum up to the amount, return None
    return None
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        self.coins[coin] = 1 + self.coins.get(coin, 0) # Add the coin to the machine
        result = make_change(coin, self.coins)  # Compute the change
        for c in result:  # Reduce the count of each coin in the result from self.coins
